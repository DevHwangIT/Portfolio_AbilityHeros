...
public class Lightninig_Trigger : MonoBehaviour {
        ...
        if (this.GetComponent<PhotonView>().isMine)
        {
            if (other.tag == "Player")
            {
                if (other.GetComponent<PhotonView>().owner.NickName != PhotonNetwork.player.NickName)
                {
                    ...
                    Ability_Trigger.Instance.pv_.RPC("User_Condition_State", PhotonTargets.All, other.GetComponent<PhotonView>().owner.NickName, Condition.Condition_State.Stun, 5, (level / 2 + 1), 1002);
                }
            }
        }
    }
