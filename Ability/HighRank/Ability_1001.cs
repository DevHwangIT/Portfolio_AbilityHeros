...
public class Ability_1001 : Ability
{
    public Ability_1001()
    {
        //순차,능력이름,능력정보,소비소울,랭크,스킬타입,쿨타임 등을 설정.
        Index = 1001;
        Name_Key = "Ability_1001_Name";
        Skill_Level = 1;
        Info_Key = "Ability_1001_Info";
        Spend_Soul = 3;
        Rank = Skill_Rank.A;
        Type = Skill_Type.Active_Skill;
        Elemental = Skill_Elemental.Wind;
        Skill_Delay = 30;
        Damege = Skill_Level * 10;
        Keep_Time = Skill_Level * 3;
    }
    
    // 부모클래스의 Active_Skill_Btn 메소드로 사용 가능여부 파악한 후 호출 됩니다.
    public override void Active_Skill()
    {
        PhotonNetwork.Instantiate("Skill_\\Skill_Resources\\Tornado", Character_State.Instance.gameObject.transform.position, Character_State.Instance.gameObject.transform.rotation, 0);
